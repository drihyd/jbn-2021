...
class Ajax_order_model extends CI_Model {
    ...
    public function _queries($table,$status=NULL,$value=NULL)
    {
        ...
        if ($date_filter=="all") {
            ...
            // $this->db->where("o.OrderDate BETWEEN CONCAT(CURDATE(), INTERVAL 10 DAY) AND NOW()");
             $this->db->where('o.OrderDate BETWEEN DATE_SUB(NOW(), INTERVAL 7 DAY) AND NOW()');
        ...
        if ($date_filter=="today") {
            ...
            $date = date('Y-m-d 00:00:00');            
            $this->db->where("o.OrderDate BETWEEN CONCAT(CURDATE(), ' 00:00:00') AND CONCAT(CURDATE(), ' 23:59:59')");
        }
        ...
        elseif ($date_filter=="monthly") {
            ...
            $this->db->where('o.OrderDate>=',$curr_date_f);
            $this->db->where('o.OrderDate<=',$curr_date_t);
            ...
            if($filter['frm_date']=="" &&  $filter['to_date']=="") {    
                ...
                $this->db->where('o.OrderDate >=',$one_week_day);
                $this->db->where('o.OrderDate <=', date('Y-m-d 23:59:59'));
           }
           else
           {   

                 $this->db->where('o.OrderDate >=', date('Y-m-d 00:00:00', strtotime($filter['frm_date'])));
                 $this->db->where('o.OrderDate <=', date('Y-m-d 23:59:59', strtotime($filter['to_date'])));
           }
