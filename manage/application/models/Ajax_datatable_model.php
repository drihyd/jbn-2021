...
class Ajax_datatable_model extends CI_Model {
    ...
    public function _query_for_products($sum)
    {
        ...
        /*$this->column_order = array(null, 'c.co_name','n.phone_name','m.ph_model_name','l.print_model_name','d.design_number', 'p.quantity'); 
        $this->column_search = array('c.co_name','n.phone_name','m.ph_model_name','l.print_model_name','d.design_number', 'p.quantity');

        if ($this->blank_cases == true) {
             $this->column_order = array(null, 'c.co_name','n.phone_name','m.ph_model_name','l.print_model_name', 'p.quantity'); 
            $this->column_search = array('c.co_name','n.phone_name','m.ph_model_name','l.print_model_name', 'p.quantity');
        } 

       if ($sum == false) {
             $this->db->select('*');
        }else if($sum ==true){
            $this->db->select("SUM(p.quantity) q");
        }
        ...
        $this->db->join('phone_models m', 'p.ph_model_id = m.ph_model_id');
        $this->db->join('print_models l', 'p.print_model_id = l.print_model_id');
        if (false == $this->blank_cases) {
            $this->db->join('designs d', 'p.design_number = d.design_number'); 
            $this->db->where('p.co_id = d.co_id');  
            $this->db->where('p.print_model_id = d.print_model_id');         
        } 
        $this->db->where('n.phone_id = m.phone_id');      
                
        if ($this->blank_cases == true) {
             $this->db->where('p.design_number', 0);
        }        */
    ...
    {    
       ...
       if(isset($filter['company']) && $filter['company'] != 'select')
        ...
        {
           $this->db->where('o.seller_id', $this->input->post('seller'));
        }*/
